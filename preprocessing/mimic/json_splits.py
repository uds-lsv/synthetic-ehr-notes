...
def main(notes_file, splits_file):
    ...
    inst = "Given a list of textual descriptions of procedure and diagnosis codes, generate a corresponding clinical discharge summary that provides comprehensive and relevant details about the patient's medical history, current condition and treatment received at the hospital."


    # Create JSON files for each split
    create_json(filtered_train, in_col='target_transcriptions', out_col='text', instruction=inst, out_name="mimic_l.json")
    create_json(filtered_test, in_col='target_transcriptions', out_col='text', instruction=inst, out_name="mimic_m.json")
    create_json(filtered_val, in_col='target_transcriptions', out_col='text', instruction=inst, out_name="mimic_s.json")
