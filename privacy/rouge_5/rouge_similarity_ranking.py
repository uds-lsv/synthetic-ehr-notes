...
parser.add_argument('--real_field',
                    help='output field of real json') # adaptation to this study
parser.add_argument('--synthpath',
                    help='path to synthetic data')
parser.add_argument('--synth_field',
                    help='output field of synth json') # adaptation to this study
...
def main(args):
    # adaptation to this study: load json fiels
    field_real = args.real_field
    with open(args.realpath) as f:
        data = json.load(f)
        training_docs = [doc[field_real].strip() for doc in data if field_real in doc and doc[field_real].strip() != '']
    
    # adaptation to this study: load json fiels
    synthfilepath = Path(args.synthpath)
    field_synth = args.synth_field
    with open(synthfilepath) as f:
        data = json.load(f)
        synth_docs = [doc[field_synth].strip() for doc in data if field_synth in doc and doc[field_synth].strip() != '']
